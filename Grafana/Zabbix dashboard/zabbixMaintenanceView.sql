...
age(to_timestamp(TP.start_date + TP.period) at time zone 'MSK', now() at time zone 'MSK') as "Remained"
    ...
    --Выбираем за какой период необходимо производить выборку начала запуска, расчитываем количество дней между стартом и текущем временем, результат дней < 14 дней
    AND date_part('day', (age(to_timestamp(TP.start_date), now() at time zone 'MSK' ))) < 14
    --Выбираем за какой период необходимо производить выборку начала запуска, расчитываем количество месяцев между стартом и текущем временем, результат месяцев < 1 месяц
